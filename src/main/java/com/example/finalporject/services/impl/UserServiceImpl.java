...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<?> sendCode(String login) {
        ...
        codeService.sendCode(UserMapper.INSTANCE.toUserDto(userFoundLogin));
    ...
    public ResponseEntity<?> getToken(String login, String code) {
        ...
        if(Objects.isNull(lastHashedCode)) {
            return new ResponseEntity<>(new ErrorResponse("You have already used NEW code. Try to create a new one"), HttpStatus.NOT_FOUND);
        }

        if(Objects.isNull(loginFound.getBlockDate()) || loginFound.getBlockDate().before(new Date())) {
            return createTokenOrBlockIfNeeded(login, code, lastHashedCode, loginFound);
        } else {
    ...
    public ResponseEntity<?> verifyUser(String token) {
        try {
            ...
            return new ResponseEntity<>("JWT has gotten expired", HttpStatus.UNAUTHORIZED);
        } catch (UnsupportedJwtException uns) {
            return new ResponseEntity<>("Entered format/configuration does not match the format expected by the application", HttpStatus.FORBIDDEN);
    ...
    // Private methods
    private ResponseEntity<?> createTokenOrBlockIfNeeded(String login, String code, Code lastHashedCode, User loginFound) {
        Request request = new Request();

        if(!BCrypt.checkpw(code, lastHashedCode.getCode())) {
            checkCode(request, lastHashedCode, loginFound);
        }
        ...
        tokenLifeSpan.add(Calendar.HOUR, 3);
    ...
    private ResponseEntity<?> checkCode(Request request, Code lastHashedCode, User loginFound) {
